...
.then(data => {
    ...
    function filterDataByPeriod() {
        ...
        for (let i = 0; i < data.length; i++) {
            for (let k = i; k < i + 1; k++) {
                ...
                current[k].textContent = data[i].timeframes[period].current + 'hrs';
                previous[k].textContent = data[i].timeframes[period].previous + 'hrs';
            }
        } 
    }
    filterDataByPeriod();
    ...
    aHref.forEach(element => element.addEventListener('click', () => {
        for (let a = 0; a < aHref.length; a++) {
			if (aHref[a].classList.contains("active")) {
